...
- [ ] **Phase 4: Summary + Scale** — Weekly Google Drive push, real MY/VN/ID sources — SG/VN/MY all real, wired end-to-end, and merged to `main` (Features 003-006, 2026-07-13); Google Drive push and ID sources still pending
...
### Phase 4: Summary + Scale `[IN PROGRESS — SG/VN/MY real sources all populated, all 8 business domains active, all merged to main (2026-07-13); ID still not started, Google Drive push still not built]`
**Goal:** Weekly synthesis pushed externally + multi-country expansion with real sources.
**Depends on:** Phase 3
**Success Criteria:**
1. Four-country daily pipeline running — SG, VN, and MY all real and, on `feature/005-domain-activation` (built on `integration/vn-my-review`), wired end-to-end together (Vietnam's `_country_mode()`/`app.py` routing now serves Malaysia's data too, since both branches are merged there). On their own original branches, VN was fully self-contained; MY alone could not switch countries (zero Python files touched) until merged with VN's routing. ID still has no real sources.
...
4. **(New sub-criterion, effectively satisfied for SG/VN/MY as of Feature 005, 2026-07-10)** All 7 business domains (BER/EDU/RCC/HLS/MFG/CTE/PSS, plus GENERAL) active and routable, not just BER/EDU/GENERAL — `_domain_mode()`, UI tabs/checkboxes, and `SUMMARY_PROMPT`'s product catalogs + opportunities gate all now cover all 8 codes. Vietnam's sources retroactively retagged with real domains (30 of 60) to make use of this; Malaysia was already tagged from Feature 004. SG itself still has no real sources tagged for the 5 newly-active verticals — domain *routing* is universal, but SG's *source coverage* under those verticals is empty by data, not by design gap.
...
- **Integration review branch (2026-07-10, `integration/vn-my-review`)** — Originally a temporary branch merging Vietnam and Malaysia together so Alfonso could review both dashboards side by side; became load-bearing once Feature 005 branched directly off it (see below), no longer just a throwaway review scaffold.
- **Feature 005 — Full Business-Domain Activation (2026-07-10, `feature/005-domain-activation`, PASSED)** — Activated all 5 remaining business domains (RCC, HLS, MFG, CTE, PSS) as first-class, routable, analyzed pipeline domains alongside BER/EDU/GENERAL: `_domain_mode()` validates 8 codes, `base.html`/`admin.html` UI expanded to 8 tabs/checkboxes, `SUMMARY_PROMPT` gained all 5 new product catalogs (verbatim from `company_context.md`) plus a 10-term broadened opportunities gate, `company_context.md`'s dormant-heading caveats removed. Retroactively retagged 30 of Vietnam's 60 sources with real business domains (mirroring Malaysia's pattern), derived from existing source descriptions only. Malaysia and Singapore needed zero source changes. Resolves the next-step item both Feature 003 and Feature 004 explicitly flagged. Built directly on `integration/vn-my-review`. See `.context/DECISIONS.md` for full rationale.
- **Feature 006 — VN/MY Accuracy & Code-Correctness Review (2026-07-13, `feature/006-vn-my-accuracy-review`, PASSED)** — Two-part audit of everything shipped in Features 003-005: report-content accuracy against live-refetched sources, and code-correctness of domain/sector/gate wiring. Found the `source_name` attribution breakage (pre-existing, predates this stack), fabricated product names, one cross-source contamination instance, filter/opportunities-gate divergence, dead RAG scoping. Applied 3 low-risk mechanical fixes; left the deeper findings flagged. **Features 003-006 merged to `main` as a single merge commit (`9e09c8f`) on 2026-07-13** — the 4-deep branch stack this section's earlier entries described is now resolved.
- **Presentation-prep session (2026-07-13)** — Same day as the merge above: domain UI consolidated to 3 visible tabs (EDU/BER/GENERAL) per country with the other 5 domains folded into GENERAL (per-item domain badges, underlying 8-domain data model untouched); leaked LLM-abstain-token placeholder signals removed from reports; "Log in as Admin" shortcut added; admin source-approval card layout fixed; VN/MY report gaps filled via live pipeline runs; both countries' source-submission PDFs cross-referenced against `config/sources.json` and confirmed complete. Presented to supervisor. See `.context/DECISIONS.md` for full rationale.
- **GitLab clone-readiness audit and two-remote split (2026-07-17)** — Repo prepared for company-wide access: missing `sentence-transformers` dependency fixed, `requirements.txt` pinned to exact versions under Python 3.12.3, `.python-version`/`.env.example` added, README rewritten and reordered so Build & Run leads. A dedicated `gitlab-main` branch was set up to push to the new `git.silversea-media.net/silversea-media/marketintelligent/ai-mi` GitLab repo, with `.claude/`/`.context/`/`CLAUDE.md` untracked on that branch only (still tracked on `main`, which keeps pushing to GitHub as before). First push to GitLab's `main` completed and verified. See `.context/DECISIONS.md` for full rationale and the ongoing publish workflow.
...
- **Full 7-domain activation (RCC/HLS/MFG/CTE/PSS) — resolved by Feature 005** (2026-07-10). All 8 domains (BER/EDU/GENERAL/RCC/HLS/MFG/CTE/PSS) are now active, validated pipeline domains — `_domain_mode()`, UI tabs/checkboxes, `SUMMARY_PROMPT` product catalogs, and the opportunities gate all cover the full set. Vietnam's sources retroactively retagged (30 of 60) to make use of it. Do not reopen this as a pending item. See `.context/DECISIONS.md` (2026-07-10 entry) for full rationale.
- **NEW (2026-07-10, from Feature 005, explicitly flagged by Alfonso, not yet scoped as a feature): accuracy and value review of Vietnam's and Malaysia's already-generated live reports.** Accuracy is checkable; "value" (how useful a given piece of information is to the company for a specific country) is inherently subjective, per Alfonso's own framing, and expected to be genuinely hard to automate or score cleanly. This is now the item most at risk of being lost across sessions — carry forward with the same prominence the domain-activation item received until it's either scoped or explicitly resolved.
- **Sibling-branch stack — resolved 2026-07-13.** Features 003-006 (plus `integration/vn-my-review`) merged to `main` as one merge commit (`9e09c8f`). Do not reopen this as a pending item.
- **`feature/002-local-llm-backend` — still unmerged, now also GitHub-only by design.** With the GitLab split (2026-07-17), `main` pushes to both `origin` (GitHub) and, via `gitlab-main`, to the company GitLab repo. `feature/002` should stay GitHub-only until it's actually verified against a real local model on real hardware — see `.context/STATE.md`'s Known Bugs section for exactly what's unverified. Branched from an older `main`, so bringing it in later needs a real merge/rebase, not just a checkout.
