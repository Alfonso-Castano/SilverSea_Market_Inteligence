...
## [2026-07-28] — Feature 009 (Daily Automation + Report Archival): PASSed, unmerged; real AOS opacity bug found and worked around; production install blocked on server access

**Decision:** Built the machinery for the production pipeline to run automatically every day at 8am across all 9 country×domain combinations (SG/VN/MY × EDU/BER/GENERAL) and permanently archive each completed report as a downloadable PDF, browsable from the app. Three new pieces: `pipeline/archive.py` (`archive_report_pdf(country_code, domain, base_url=None)` — logs into the real, already-running Flask app as the viewer role via Playwright, navigates to the report page, and renders it to `data/archive/{country}/{domain}/{YYYY-MM-DD}.pdf` using headless Chromium's `page.pdf()`, reusing the app's existing print CSS rather than a second rendering path); `scripts/daily_pipeline.py`/`.sh` (one sequential wrapper script — not 9 separate crontab lines — that subprocess-invokes `main.py --country=X --domain=Y --no-email` for each of the 9 combinations in turn, continuing past a failed combination rather than aborting, archiving immediately after each successful one, logging every outcome to `data/logs/daily_pipeline.log`); a new "Report Archive" section on `/internals` (`app.py`'s `_list_archives()`/`download_archive()` + `templates/internals.html`), gated by ordinary login (no admin requirement, matching `/internals`'s existing access model) with three-layer input validation (country whitelist via `load_sources()`, domain whitelist, filename regex) before touching the filesystem.
**Rationale:** Direct implementation of Alfonso's explicit requirements, locked in `.context/features/009-daily-automation-archival/CONTEXT.md`'s Implementation Decisions: SSH key-based server access (a dedicated, independently-revocable `~/.ssh/aimi_prod_deploy` ed25519 keypair, not Alfonso's personal key) over "human installs it" or deciding later; Playwright + headless Chromium over a server-side HTML-to-PDF library, specifically to get true visual parity with the real report page for zero duplicated rendering logic, accepting the added Chromium-binary dependency weight; sequential (not parallel) wrapper execution specifically to avoid concurrent ChromaDB writes across simultaneous `main.py` processes, echoing an already-documented transient ChromaDB concurrent-access issue in `.context/STATE.md`'s known bugs; archive browsing placed on `/internals` (Claude's own default judgment, flagged for Alfonso's confirmation at plan review, not overridden) because archived reports are "what's happened with this system" observability, not an admin-only action like source approval.

**Decision — a real, previously-undocumented bug in existing (pre-Feature-009) code was found and fixed along the way, not part of the original plan:** `templates/report.html`'s major sections (`#sectors`, `#opportunities`, `#competition-risks`, `#data-sources`) all carry AOS 2.3.1's `data-aos="fade-up"`, which keeps them at `opacity:0` until an `IntersectionObserver` marks them `.aos-animate` on scroll-into-view — never triggered by a single-shot headless render that never scrolls, and `static/style.css`'s `@media print` block has no override for it. The first execution attempt of Task 002 discovered this the hard way: the archived PDF contained only ~1,400 characters of extractable text (executive summary and footer only). Fixed, for the automated archival path only, by resizing the Playwright viewport to `document.body.scrollHeight` (measured after the pre-existing entity-group expansion step, since that changes page height) plus a 300ms settle wait, before calling `page.pdf()` — brings every AOS-tagged section into the observed viewport at once without touching `report.html`/`animations.js`/`style.css`. Confirmed effective: extracted text went from ~1,400 to ~25,801 characters with real signal-card content present, independently re-verified in `/feature-verify` against the executor's own already-generated PDF.
**Rationale:** This is a genuine, previously-unknown defect in code this feature didn't write — the existing, human-facing "Export PDF" button (`static/animations.js`'s `initPdfExport()`) shares the identical underlying rendering path and would produce the same mostly-blank result for any user who clicks it without first scrolling through the entire report. Feature 009 fixed it only for its own new automated path (the viewport-resize trick lives in `pipeline/archive.py`, not in any file the manual button touches) — the manual button itself remains unfixed and is recorded as a new, separate known bug in `.context/STATE.md`, not silently folded into this feature's own scope.

**Decision:** Explicitly did not attempt actual installation on the production server — no SSH access was confirmed at any point this session. A dedicated SSH deploy keypair (`~/.ssh/aimi_prod_deploy`, ed25519, generated this session, private half never leaving this dev machine or entering the repo/chat) had its public key shared out-of-band with teammate leo.li; as of this entry, still awaiting his reply with the server hostname/IP, the deploy username the key should be added under, and confirmation it's actually in that user's `authorized_keys`. Two useful facts were confirmed directly by leo.li this session and are worth recording rather than re-deriving later: the production server is aaPanel-managed (consistent with the existing `/www/wwwroot/ai-mi` path already referenced in `deploy/start.sh`/the bilingual deployment docs from the 2026-07-21 incident), and it runs on Singapore server time — meaning the 8am cron/scheduled-task target is already server-local, no timezone-conversion math needed once access is granted.
**Rationale:** CONTEXT.md's own Open Questions section named this as a hard blocker for the cron-install task specifically, not for building/testing the wrapper script and PDF logic locally first — and that's exactly how it played out. `/feature-verify`'s review treated the missing server install as a transparently-documented external blocker, not a shortfall in what this feature was able to build and verify, and PASSed on that basis (matching the precedent Feature 007 set for its own DeepSeek-billing gap and Feature 008 set for its OpenRouter/NVIDIA China-reachability gap — a real external dependency outside Claude's control doesn't block a PASS as long as it's honestly disclosed, not glossed over).

**Decision:** Branch `feature/009-daily-automation-archival` PASSed `/feature-verify` (fresh syntax/import checks, an independently-written mocked wrapper control-flow test, a live re-run of the `/internals` archive security checks against a freshly booted dev server, and a gitignore/history audit confirming no archive/log file was ever accidentally committed) but has **not** been merged to `main` (or from there to `gitlab-main`) as of this entry — a merge to both remotes is expected immediately after this context refresh, in the same session, per Alfonso's direction.
**Rationale:** Same standing practice as Features 007/008's own entries — a PASS unblocks merging, it doesn't trigger it automatically. Recorded plainly so a future session checks `git rev-parse main gitlab-main` rather than assuming this entry's "not yet merged" framing is still current.
